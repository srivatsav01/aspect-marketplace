...
function AddAspectModal({ aspect, onClose }) {
  ...
  const handleBindAspect = async() => {
    ...
    try {
        ...
        let signedTx = await web3.eth.sendTransaction(tx).on('transactionHash', (hash) => {
            console.log('Transaction hash:', hash);
          })
          .once('confirmation', (confirmationNumber, receipt) => {
            // This handler will be called only for the first confirmation
            console.log('Transaction confirmed:', receipt);
            alert(`yayy you've added ${aspect.name} to your contract`);
  ...
  return (
    ...
    <p className="text-sm text-gray-600 my-2">Enter the contract address and ABI of your contract address</p>
